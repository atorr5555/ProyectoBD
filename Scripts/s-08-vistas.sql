...
create or replace view view_vehiculo(
  vehiculo_id,anio,numero_serie,nombre,num_placa
) as 
select v.vehiculo_id,v.anio,v.numero_serie,pp.nombre,p.num_placa
...
select p.propietario_id,p.nombre,l.num_licencia,n.cantidad
...
create or replace view vehiculo_carga(
  vehiculo_id,capacidad_toneladas,anio,nombre
) as 
select c.vehiculo_id,c.capacidad_toneladas,v.anio, m.nombre
...
create or replace view vehiculo_transporte(
  vehiculo_id,num_pasajeros_sentados,anio
) as 
select t.vehiculo_id, t.num_pasajeros_sentados,v.anio
...
select pa.vehiculo_id,pa.num_bolsas_aire,v.numero_serie,p.nombre
